...
var past_purchases: [Double] = [41.10, 10.45, 0.10, 90.102, 49.2, 48.1, 69.01, 1, 2, 0]

for purchase in past_purchases {
    print("Purchase: $\(purchase)")
}

for i in 0..<past_purchases.count {
    past_purchases[i] += 0.10
}

for i in (0..<past_purchases.count).reversed() {
    let formatted_price = String(format: "%.2f", past_purchases[i])
    print("Purchase Updated: $\(formatted_price)")
}
